...
/* Get the rank for single-letter subsets, lower value meaing higer
   priority.  */
...
single_letter_subset_rank (char ext)
{
  ...
  if (order_a == NULL)
    /* If got an unknwon extension letter, then give it an alphabetical
       order, but after all known standard extension.  */
    rank = strlen (all_ext) + ext - 'a';
  else
    rank = (int)(all_ext - ext_pos);

  return rank;
}

/* Get the rank for multi-letter subsets, lower value meaing higer priority.  */
...
subset_cmp (const std::string &a, const std::string &b)
{
  ...
  if (a_len  == 1 && b_len == 1)
    {
      int rank_a = singleletter_subset_rank (a[0]);
      int rank_b = singleletter_subset_rank (a[0]);
