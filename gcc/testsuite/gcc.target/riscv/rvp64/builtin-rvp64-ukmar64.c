/* This is a test program for ukmar64 instruction.  */

/* { dg-do run { target { riscv64*-*-* } } } */

#include <p_ext_intrinsic.h>
#include <stdlib.h>
#include <stdint.h>

#if defined(__riscv_zpsf) && !defined(__riscv_32e)
static __attribute__ ((noinline))
unsigned long long ukmar64 (unsigned long long rd, unsigned long ra, unsigned long rb)
{
  return __rv__ukmar64 (rd, ra, rb);
}

int
main ()
{
  unsigned long long a = ukmar64 (0x1, 0xffff0000ffff0000ll, 0x0001ffff0001ffffll);

  if (a != 0x3fffa00020001ll)
    abort ();
  else
    exit (0);
}
#else
int main(){return 0;}
#endif
